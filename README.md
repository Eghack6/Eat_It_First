...
- **一键加入链接** — 分享家庭专属链接，对方打开只需输入昵称即可加入；同一链接可多台设备重复使用，随时可作废换新
- **邀请码加入** — 也可凭 10 位邀请码加入；同一昵称在所有设备上自动归为同一身份
...
| POST | `/api/families/join-link`     | 凭加入链接（免登录）加入家庭      |
| GET  | `/api/families/current`       | 当前家庭与成员                    |
| GET  | `/api/families/join-link`     | 获取家庭加入链接 token            |
| POST | `/api/families/join-link/rotate` | 轮换加入链接 token            |
...
除公开的登录注册接口（`/api/families`、`/api/families/join`、`/api/families/join-link` 的 POST）外，所有 `/api/*` 路由均需携带 `Bearer` token（创建/加入家庭时签发）。认证采用每设备 token，仅存 SHA-256 哈希。
