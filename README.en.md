...
- **One-click join link** — share a family-specific link; recipients just enter a nickname to join, reusable across unlimited devices, revocable anytime
...
| POST   | `/api/families/join-link`     | Join a family via share link (public) |
| GET    | `/api/families/current`       | Current family + members           |
| GET    | `/api/families/join-link`     | Get the family's join-link token   |
| POST   | `/api/families/join-link/rotate` | Rotate the join-link token      |
...
src/
  ...
  auth.js       Family / member / device auth, invite codes, join links
